...
void writeCharToSegment(uint8_t segment, char character){
  ...
  if(character < 'A' || character > 'Z'){
    value=SPACE;
  }else{ // enkel waarden tussen 'A' en 'Z'
...
// Zet alleen segment 'a' aan (bovenste horizontale lijn) op gekozen segment
void writeLineTop(uint8_t segment) {
  cbi(PORTD, LATCH_DIO);
  shift(~SEG_A, MSBFIRST);           // segment a aan (bit 0 = 0), rest uit (bit 1..7 = 1)
  shift(SEGMENT_SELECT[segment], MSBFIRST);
  sbi(PORTD, LATCH_DIO);
}

// Zet alleen segment 'g' aan (midden horizontale lijn) op gekozen segment
void writeLineMiddle(uint8_t segment) {
  cbi(PORTD, LATCH_DIO);
  shift(~SEG_G, MSBFIRST);
  shift(SEGMENT_SELECT[segment], MSBFIRST);
  sbi(PORTD, LATCH_DIO);
}

// Zet alleen segment 'd' aan (onderste horizontale lijn) op gekozen segment
void writeLineBottom(uint8_t segment) {
  cbi(PORTD, LATCH_DIO);
  shift(~SEG_D, MSBFIRST);
  shift(SEGMENT_SELECT[segment], MSBFIRST);
  sbi(PORTD, LATCH_DIO);
} 
